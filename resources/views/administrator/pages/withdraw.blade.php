					...
					<tbody>
						...
						<tr>
							...
							<td style="vertical-align: middle;"><a href="https://vk.com/{{ $withdrow->user->login }}"
									...
									data-target="#user_edit" href="/administrator/user/{{ $withdrow->user->id }}/edit" style="display: inline-block;
																									cursor: pointer;">Инфо</div>
							</td>
							<td style="vertical-align: middle;">{{ $withdrow->manager }}</td>
							...
							<td style="vertical-align: middle;">@if(isset($withdrow->status) && isset($withdrow->id))
								@if($withdrow->status == 0)<a class="btn blue btn-sm" data-toggle="modal"
									...
									href="/administrator/withdraw/{{ $withdrow->id }}/edit">Редактировать</a>@endif @endif</td>
						</tr>
						@endforeach
						@endif
					</tbody>
