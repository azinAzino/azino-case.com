...
<div class="row">
	...
	<div class="col-md-12">
		<div class="bs-callout bs-callout-danger">
			...
			<p>Токен от страницы используется при генерации реферальной ссылки для пользователя, рекомендуем
				настаятельно не брать токен с основного аккаунта ВКонтакте, а создать твинк. При переходе
				по ссылке для получения токена, в браузерной строке появится URL типа: <br>
				<pre>http://api.vk.com/blank.html#access_token=ac467e477285fe4144ad691886785e4551cfb0e90096fbb7ecde4b817ea1b8e22e4fa167e9048b2e817d6&expires_in=0&user_id=XXXXXXXXX</pre>
				<br>
				Из которой нужно скопировать код, идущий после <code>#access_token=</code> и до
				<code>&expires_in</code><br>
				В нашем случае, это код:<br>
				<pre>ac467e477285fe4144ad691886785e4551cfb0e90096fbb7ecde4b817ea1b8e22e4fa167e9048b2e817d6</pre>
			</p>
		</div>
	</div>
	<div class="col-md-12">
		<div class="bs-callout bs-callout-danger">
			...
			<p>В настройках кассы укажите следующие ссылки:<br>
				<b>URL оповещения: <code>{{ $_SERVER['HTTP_HOST'].'/checkpayment' }}</code>, метод:
					<code>POST</code><br>
					URL успешной / неуспешной оплаты: <code>{{ $_SERVER['HTTP_HOST'].'/success' }}</code>, метод:
					<code>GET</code></b></p>
		</div>
	</div>
</div>
	...
	<div class="col-md-12">
		<div class="portlet light bordered">
			<div class="portlet-body">
				<form method="post" action="/administrator/settings/save" class="horizontal-form">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									...
									<input type="number" class="form-control" placeholder="20" name="min_dep"
										value="{{ $settings->min_dep }}" onchange="if (this.value < 0) this.value=0">
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">Сумма минимального вывода</label>
									<input type="number" class="form-control" placeholder="100" name="min_width"
										value="{{ $settings->min_width }}" onchange="if (this.value < 0) this.value=0">
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
									...
									<input type="text" class="form-control" placeholder="https://vk.com/cheap.scripts"
										name="vk_group" value="{{ $settings->vk_group }}">
								</div>
							</div>
							<div class="col-md-4">
								<div class="form-group">
									<label class="control-label">Токен от страницы VK (<a href="http://goo.gl/KseyIT"
											target="blank">получить</a>)</label>
									<input type="text" class="form-control"
										placeholder="ac467e477285fe4144ad691886785e4551cfb0e90096fbb7ecde4b817ea1b8e22e4fa167e9048b2e817d6"
										name="vk_token" value="{{ $settings->vk_token }}">
								</div>
							</div>
							<div class="col-md-4">
								<div class="form-group">
									<label class="control-label">Процент отчисления пригласившему</label>
									<input type="number" class="form-control" placeholder="5" name="tttk"
										value="{{ $settings->ref_percent }}">
								</div>
							</div>
						</div>
						<div class="row">
						...
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
									...
									<select class="form-control" tabindex="1" name="payment_type"
										value="{{ $settings->payment_type }}">
										<option value="1" @if($settings->payment_type == 1) selected @endif>Payeer
										</option>
										<option value="0" @if($settings->payment_type == 0) selected
											@endif>Free-Kassa(FK)</option>
									</select>
								</div>
							</div>
							<div class="col-md-4">
								<div class="form-group">
									...
									<input type="text" class="form-control" placeholder="Секретный ключ #1"
										name="pt_shopid" value="{{ $settings->pt_shopid }}">
								</div>
							</div>
							<div class="col-md-4">
								<div class="form-group">
									...
									<input type="text" class="form-control" placeholder="Секретный ключ #2"
										name="pt_secret" value="{{ $settings->pt_secret }}">
								</div>
							</div>
						</div>
						<div class="row">
